...
function onInit() {
  ...
  var elEx = document.querySelector('.exterminator-container')
  var elBtn = document.querySelector('.exterminator-btn')
  var elInd = document.querySelector('.exterminator-indicator')
...
function updateFlagCounter() {
  ...
  var remaining = gRoundMineCount - gGame.markedCount
...
function updateFlagCounterAtEnd() {
  var elCounter = document.querySelector('.flags-counter')
  var finalMinesLeft = Math.max(0, gRoundMineCount)
...
function onCellClicked(i, j, elCell) {
  ...
  if (gIsHintMode) {
    ...
    gIsHintMode = false
    gHintsCount--
    var activeHint = document.querySelector('.hint.active')
    if (activeHint) {
...
function expandReveal(ci, cj, board) {
  for (var i = ci - 1; i <= ci + 1; i++) {
    ...
    for (var j = cj - 1; j <= cj + 1; j++) {
      ...
      var elN = document.querySelector(`[data-i="${i}"][data-j="${j}"]`)
...
function checkVictory() {
  ...
  var safeCellsToReveal = totalCells - gRoundMineCount
...
function onExterminateClick() {
  ...
  setTimeout(() => {
    ...
    var toRemove = Math.min(3, stillHiddenMines.length)
    for (var k = 0; k < toRemove; k++) {
      var idx = getRandomInt(0, stillHiddenMines.length)
      var pos = stillHiddenMines.splice(idx, 1)[0]
...
function startTimer() {
  ...
  gTimerInterval = setInterval(() => {
    ...
    var total = Math.floor(gGame.secsPassed * 1000)

    var m = Math.floor(total / 60000)
    var s = Math.floor((total % 60000) / 1000)
    var h = Math.floor((total % 1000) / 10)
